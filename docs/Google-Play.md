...
4. **Icons: DONE.** The manifest serves PNG icons including the 512 px
   maskable one Android requires (`/icon-192.png`, `/icon-512.png`,
   `/icon-512-maskable.png`), and all three plus the favicon
   (`/icon.svg`) carry the SportFits aperture mark: six emerald-gradient
   blades around an ink hub on a dark tile, matching the in-app wordmark
   (components/brand/sportfits-mark.tsx). Go to **pwabuilder.com**, enter
   the production URL, and let it score the site (manifest, service
...
7. **Listing assets: DONE.** `store-assets/` holds the SportFits set on the
   aperture-mark brand: `feature-graphic-1024x500.png` (mark + two-tone
   wordmark + tagline on ink) and five 1080x1920 dark-theme screenshots of
   the current app (hub sport-picker, scored results dashboard, the rabbit
   hole, a drill guide with its "what good looks like" figure, and the golf
   upload screen with camera diagrams). In the Play Console: **Create app**,
   fill in the store listing (name, short and full descriptions) and upload
   those files.
